...
export const loginUser = (userData) => dispatch => {
    ...
    dispatch(LogOut())
    axios
        .post(`https://xgenbox.azurewebsites.net/api/users/login`, {email:userData.email, password:userData.password})
        .then(res => {
...
const data = {email, name:familyName+' '+givenName,avatar:photo, googleId:id, tokenId:idToken}
    ...
    axios.get(`https://xgenbox.azurewebsites.net/api/users/getUserByEmail/${info.user.email}`)
    .then(res => {
        // console.log(res)
        dispatch({
            type: SET_USER,
